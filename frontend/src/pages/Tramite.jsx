...
import { obtenerTramite, validarDocumento, generarDocumento, descargarDocumento, enviarTramiteOnline } from '../repositories/tramitesRepository'
import { archivoABase64 } from '../utils/fileUtils'
import { useAuth } from '../context/AuthContext'
import { useApp } from '../context/AppContext'
import MapaOficina from '../components/MapaOficina'
...
export default function Tramite() {
  ...
  const handleEnviarSolicitud = async () => {
    if (!currentUser) {
      setErrorEnvio('Debes iniciar sesión para enviar tu solicitud')
      return
    }
    ...
    try {
      // Preparar documentos validados
      const documentosValidados = tramite.documentos_requeridos.map((doc) => {
        const estado = checklist[doc.id] || {}
        return {
          documento_id: doc.id,
          nombre: doc.nombre,
          tipo: doc.tipo,
          validado: estado.validado || false,
          resultado_ocr: estado.resultado || null
        }
      })

      // Enviar solicitud
      const response = await enviarTramiteOnline({
        programaId: parseInt(id),
        datosUsuario: formDatos,
        documentosValidados
      })
      ...
      setErrorEnvio(err.response?.data?.error || 'Error al enviar la solicitud. Intenta de nuevo.')
    } finally {
  ...
  return (
    <div className="container mx-auto px-4 py-12 max-w-4xl">
      ...
      {/* Grupo A — Mapa de ubicación del módulo presencial */}
      {!esGrupoB && tramite.modulo_atencion && (
        <MapaOficina modulo={tramite.modulo_atencion} />
      )}
      ...
      {/* Botón para enviar solicitud (Grupo B) */}
      {esGrupoB && completados === total && total > 0 && !solicitudEnviada && (
        <div className="bg-gradient-to-br from-emerald-50 to-green-50 rounded-2xl shadow-lg border-2 border-emerald-200 p-8 mb-8">
          <div className="text-center mb-6">
            ...
            <h3 className="text-2xl font-black text-gray-900 mb-2">¡Todo listo para enviar!</h3>
            <p className="text-gray-600">
              Has completado todos los pasos. Haz clic en el botón para enviar tu solicitud.
            </p>
          </div>
          ...
          {errorEnvio && (
            <div className="mb-4 p-4 bg-red-50 border border-red-200 rounded-xl flex items-start gap-3">
              ...
              <p className="text-sm text-red-700 font-medium">{errorEnvio}</p>
            </div>
          )}
          ...
          <button
            ...
            className="w-full py-4 rounded-xl text-white font-bold text-lg transition-all disabled:opacity-50 hover:opacity-90 hover:shadow-xl flex items-center justify-center gap-3"
          ...
          <p className="text-xs text-gray-500 text-center mt-4">
            Al enviar, tu solicitud será revisada por un administrador
          </p>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black bg-opacity-50 animate-fade-in">
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-scale-in">
            <div className="text-center">
              ...
              <h3 className="text-3xl font-black text-gray-900 mb-3">¡Solicitud Enviada!</h3>
              <p className="text-gray-600 mb-6">
                Tu solicitud ha sido enviada exitosamente. Recibirás una notificación cuando sea revisada.
              </p>
              <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 mb-6">
                <p className="text-sm text-blue-800">
                  <span className="font-bold">Próximos pasos:</span><br />
